...
while visualizer:
    for i in range(N):
        ...
        angle = np.arccos(np.dot(v_this, (x_that-x_this).T) /
                          (np.linalg.norm(v_this) * np.linalg.norm((x_that-x_this), axis=1)))
        # 各力が働く範囲内の個体のリスト
        coh_agents_x = x_that[(distance < COHESION_DISTANCE)
                              & (angle < COHESION_ANGLE)]
        ...
        # 各力の計算
        dv_coh[i] = COHESION_FORCE * \
            (np.average(coh_agents_x, axis=0) -
             x_this) if (len(coh_agents_x) > 0) else 0
        dv_sep[i] = SEPARATION_FORCE * \
            np.sum(x_this - sep_agents_x,
                   axis=0) if (len(sep_agents_x) > 0) else 0
        dv_ali[i] = ALIGNMENT_FORCE * \
            (np.average(ali_agents_v, axis=0) -
             v_this) if (len(ali_agents_v) > 0) else 0
        dist_center = np.linalg.norm(x_this)  # 原点からの距離
        dv_boundary[i] = - BOUNDARY_FORCE * x_this * \
            (dist_center - 1) / dist_center if (dist_center > 1) else 0
    # 速度のアップデートと上限/下限のチェック
    v += dv_coh + dv_sep + dv_ali + dv_boundary
    # エサへの吸引力を加える
    v += PREY_FORCE * (prey_x - x) / \
        np.linalg.norm((prey_x - x), axis=1, keepdims=True)**2
